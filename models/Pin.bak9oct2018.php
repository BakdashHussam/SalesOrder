...
class Pin extends Model
{
	...
	private function setConfig()
	{
		...
		$this->config = [
			'oauth' => [
				...
				'consumer_key'     => 'IRL82UVJESJSD96QUPUOBDZKAXBCES',
				'consumer_secret'  => 'VDASA5DXXCHDE6T8RRHGPRXCF8RLKH',
	...
	public function addNewSalesOrder($col)
	{
		...
		if(count($sql) && isset($sql[0]["sale_order_no"]) && $sql[0]["sale_order_no"]!="")
		{
			$queryString = "insert into sale_order 
									(sale_order_no, sale_order_date, customer_id, customer_code, customer_name, discount_rate,discount_rate_ori,discount,term,tax,approval_status,create_date,create_user)
									select CAST(max(sale_order_no) AS bigint)+1 ,'".$orderdate."','".$customer_id."','".$customer."','".$customer_name."','".$discount_rate."','".$discount_rate_ori."','".$discount_amount."','".$term."','".$tax."',0,getdate(),'".Yii::$app->user->identity->username."' from sale_order
									";
		}
		else
		{
			$queryString = "insert into sale_order 
									(sale_order_no, sale_order_date, customer_id, customer_code, customer_name, discount_rate,discount_rate_ori,discount,term,tax,approval_status,create_date,create_user)
									values
									('10000000','".$orderdate."','".$customer_id."','".$customer."','".$customer_name."','".$discount_rate."','".$discount_rate_ori."','".$discount_amount."','".$term."','".$tax."',0,getdate(),'".Yii::$app->user->identity->username."')";
		}
	...
	public function getSalesOrderDetail($id)
	{
		...
		$sql3 = Yii::$app->db->createCommand(
											"select credit_note_id, total_amount
											from credit_note
	...
	public function getSalesOrderListing()
	{
		...
		foreach($query as $key=>$value)
		{
			$json .= ",['".$value["sale_order_no"]."',new Date(".date("Y",strtotime($value["sale_order_date"])).",".date("m",strtotime($value["sale_order_date"])).",".date("d",strtotime($value["sale_order_date"]))."),'".$value["customer_name"]."','".$thestatus[$value["approval_status"]]."','".$value["sale_order_id"]."']";
		}
		//var_dump($sql->getText());exit;
	...
	public function getCreditNoteListing()
	{
		...
		foreach($query as $key=>$value)
		{
			$json .= ",['".$value["credit_note_no"]."',new Date(".date("Y",strtotime($value["credit_note_date"])).",".date("m",strtotime($value["credit_note_date"])).",".date("d",strtotime($value["credit_note_date"]))."),".number_format(floatval($value["total_amount"]),"3",".","," ).",'".$thestatus[$value["approval_status"]]."','".$value["credit_note_id"]."']";
		}
		//var_dump($sql->getText());exit;
		return substr($json,1);
	}
	...
	{		
			...
			foreach($card_check_status as $key=>$value)
			{
				if(isset($list_to_update_from_excel[$value["batchid"]]))
				{
					foreach($list_to_update_from_excel[$value["batchid"]] as $key2=>$value2)
					{
						...
						{
							$error_card_list .= "serial: ".$value["cardserial"].", code: ".$value["cardcode"]." | ";
						}
					}
				}
			}
		...
		try
		{
			$sql = Yii::$app->db->createCommand(
									...
									(credit_note_no, sale_order_id, discount_rate,discount,total_amount,credit_type,create_date,create_user,approval_status)
									values
									('".$credit_note_no."','".$id."','".$discount_rate."',".$discount.",".$total_amount.",'".$creditnoteType."',getdate(),'".Yii::$app->user->identity->username."',0)
									");
			$query = $sql->execute();
			$last_id = Yii::$app->db->getLastInsertID();
		}
	...
	{		
		...
		if($last_id>0)
		{
			...
			if($flag)
			{
				if($creditnoteType=="1")
				{
					$sql = Yii::$app->db->createCommand("update generated_code set credit_note='".$last_id."' where sale_order_id =".$id);
					$query = $sql->execute();
				}
				else if($creditnoteType=="2")
				{
					...
					if(count($list_to_update_from_excel)>0)
					{
						foreach($list_to_update_from_excel as $key=>$value){
							foreach($value as $key2=>$value2){
								$sql = Yii::$app->db->createCommand("update generated_code set credit_note='".$last_id."' where sale_order_id =".$id." and cardserial='".$value2["cardserial"]."' and cardcode='".$value2["cardcode"]."'");
								$query = $sql->execute();
							}
						}
					}
	...
	*/
		...
		if(count($list)>0)
		{
			...
			if(!$empty)
			{
				...
				$sql = Yii::$app->db2->createCommand(
													"select cardserial,cardcode,batchid,ownerid,ownermemberid,ownerip,used_date
													from temp_cubicards
	...
	private function pushGeneratedCodeToDB($id)
	{
		...
		if($query && count($query)>0)
		{
			...
			foreach($query as $key=>$value)
			{
				$insert_statement .= "insert into temp_cubicards (cardserial,cardcode,batchid,distroid,issued_by,issued_date,expire_date,invoice_no,remarks,blocked)
										values (
										'".$value["cardserial"]."',
										'".$value["cardcode"]."',
										'".$value["product_code"]."',
	...
	private function DeactivatedCodeFromDB($id)
	{
		...
		if($query && count($query)>0)
		{
			...
			foreach($query as $key=>$value)
			{
				$update_statement .= "update temp_cubicards set blocked='2' where cardserial='".$value["cardserial"]."' and cardcode='".$value["cardcode"]."'";
			}
			$connection = Yii::$app->db2;
			$transaction = $connection->beginTransaction();
			try
	...
	public function getDirectTopup($col)
	{
		$return = array();
		$return[0] = array();
		$data_date = date("Y-m-01",strtotime($col["orderdate"]));
		...
		switch( strtolower($col["customer_code"]) )
		{
			case "maxi" : $pgw = "maxis";
				break;
			case "offg" : $pgw = "offgamers";
				break;
			case "indo" : $pgw = "indomog";
				break;
			case "pt24" : $pgw = "unipay";
				break;
			case "eage" : $pgw = "gb";
				break;
			case "acs" : $pgw = "yougopay";
				break;
			case "payp" : $pgw = "paypal";
				break;
			case "g2a" : $pgw = "g2a";
				break;
			case "m2u" : $pgw = "mbb2u";
		...
		switch($pgw)
		{
			...
			case "yougopay" :
							$sql_string = "select b.trn_product_price as value, (b.trn_product_price*10) as cubits, sum(b.trn_product_qty) as qty
											...
											where (a.create_date between DATEADD(MONTH,-1,'".$data_date."') and '".$data_date."')
											and a.trn_ecomm_provider ='".$pgw."' and flag_processed='99' and a.trn_id = b.trn_id
			...
			case "unipay" :
							$sql_string = "select  trn_grand_total as value, trn_approval_code as cubits, count(trn_id) as qty
											from trn_purchase
											where (create_date between DATEADD(MONTH,-1,'".$data_date."') and '".$data_date."')
											and trn_ecomm_provider ='".$pgw."' and flag_processed='99'
	...
	public function pushAllocation()
	{
		$game = array();
		$flag = 0;
		$game_sql = Yii::$app->db->createCommand("select game_code, game_name, game_code_xero from games where game_code_xero is not null");
		$game_query = $game_sql->queryAll();
		if($game_query && count($game_query)>0)
		{
			foreach($game_query as $key=>$value){
				$game[$value["game_code"]]["xero"] = $value["game_code_xero"];
				$game[$value["game_code"]]["name"] = $value["game_name"];
			}
		}
		
		$data_date = date("m/1/Y 00:00:00.000");
		$sql = Yii::$app->db->createCommand("select allocation_game_code as game_code, sum(allocation_cubits) as amount 
											from allocation
											where create_date between DATEADD(MONTH,-1,'".$data_date."') and '".$data_date."'
			...
			foreach($query as $key=>$value)
			{
				if(isset($game[$value["game_code"]]))
				{
				  ...
				  // set credit line
					...
					$credit->setLineAmount($value["amount"])
