...
class Pin extends Model
{
	...
	public function getCustomer()
	{
		...
		*/
		...
		foreach ($single["Contacts"] as $contact) {
			$item = $xero->loadByGUID('Accounting\\Contact', $contact["ContactID"]);
			$temp = array();
			$temp["customer_id"] = $item["ContactID"];
			$temp["customer_name"] = $item["Name"];
			$temp["customer_code"] = $item["AccountNumber"];
			$temp["currency"] = $item["DefaultCurrency"];
			$temp["discount"] = $item["Discount"];
			$temp["term"] = $item["PaymentTerms"]["Sales"]["Day"];
			$customer_list[] = $temp;
        }
	...
	public function getSalesOrderListing()
	{
		...
		, sale_order_no, sale_order_date, customer_name, approval_status from sale_order order by sale_order_date desc");
		$query = $sql->queryAll();
		$json="";
		$thestatus = array("0"=>"PENDING","1"=>"APPROVED","-1"=>"REJECTED");
		foreach($query as $key=>$value)
		{
			$json .= ",['".$value["sale_order_no"]."','".$value["sales_order_mode"]."',new Date(".date("Y",strtotime($value["sale_order_date"])).",".(intval(date("m",strtotime($value["sale_order_date"])))-1).",".date("d",strtotime($value["sale_order_date"]))."),'".$value["customer_name"]."','".$thestatus[$value["approval_status"]]."','".$value["sale_order_id"]."']";
		}
		//var_dump($sql->getText());exit;
	...
	public function setApproval($id,$status,$desc="")
	{
		...
		if($xeroReturn && $approval_status==1 && $status=='yes')
		{
	...
	public function getPendingDTU()
	{
		...
		$query = $sql->queryAll();
		$todayMonth = date("Y-m-01 00:00:00.000");
		$sql2 = Yii::$app->db3->createCommand("select trn_ecomm_provider as pgw, theDate, sum(ttl) as ttl from 
												(
												select trn_ecomm_provider, trn_grand_total as ttl,DATENAME(m, create_date) + ' ' +  CONVERT(varchar(4), DATEPART(year, create_date)) as theDate 
												from trn_purchase where (create_date between DATEADD(MONTH,-1,'".$todayMonth."') and '".$todayMonth."') and flag_processed='99' and flag_credited=1
												) as x
												group by trn_ecomm_provider, theDate
											");
		$query2 = $sql2->queryAll();
	...
	public function getDirectTopup($col)
	{
		$return = array();
		$return[0] = array();
		$data_date = date("Y-m-01",strtotime($col["dtu_month"]));
		...
		switch($pgw)
		{
			...
			case "yougopay" :
							$sql_string = "select b.trn_product_price as value, (b.trn_product_price*10) as cubits, sum(b.trn_product_qty) as qty
											...
											where (a.edit_date between '".$data_date." 00:00:00.000' and DATEADD(MONTH,+1,'".$data_date." 00:00:00.000'))
			...
			case "unipay" :
							$sql_string = "select  trn_grand_total as value, trn_approval_code as cubits, count(trn_id) as qty
											from trn_purchase
											where (edit_date between '".$data_date." 00:00:00.000' and DATEADD(MONTH,+1,'".$data_date." 00:00:00.000'))
											and trn_ecomm_provider ='".$pgw."' and flag_processed='99' and flag_credited='1'
	...
	public function pushAllocation()
	{
		$game = array();
		$flag = 0;
		$game_sql = Yii::$app->db4->createCommand("select game_code, game_name, game_code_xero from game where game_code_xero is not null");
		$game_query = $game_sql->queryAll();
		if($game_query && count($game_query)>0)
		{
			foreach($game_query as $key=>$value){
				$game[strtolower($value["game_code"])] = array("xero" => $value["game_code_xero"],"name"=> $value["game_name"]);
			}
		}
		...
		$sql = Yii::$app->db5->createCommand("select allocation_game_code as game_code, sum(allocation_cubits) as amount 
											...
											and allocation_login_id not in ('7311931')
		...
		$sql = Yii::$app->db5->createCommand("select allocation_game_code as game_code, sum(allocation_cubits) as amount 
											from allocation
											where (edit_date between '10/1/2018 00:00:00.000' and '10/29/2018 00:00:00.000')
											and allocation_sid = 1 and (allocation_product_id = '100' or allocation_product_id = '11' or allocation_product_id = '9')
											and allocation_login_id not in ('7311931')
											group by allocation_game_code
											");
		...
		*/
		...
		//if(1==0)
		{
			...
			$journal->setNarration(date("Y-m-d",strtotime($data_date . "-1 days")) . ' CMP Allocation')
			//$journal->setNarration( ' 1 Oct 2018 to 28 Oct 2018 CMP Allocation')
			  ->setStatus('POSTED');
