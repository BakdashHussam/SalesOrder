...
<div class="row-fluid sortable">
	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
				...
				$trx_fee_disc = 0;
				$trx_fee_disc_tax = 0;
				$discount = (isset($sales_order_data["discount"]))?$sales_order_data["discount"]:"0.00"; 
					...
					foreach($sales_order_details as $key=>$value){
						if($value["product_code"]=="TRX FEE")
						{
							...
							if($tax=="Inclusive")
							{
								...
								$amount = $amount-$taxed;
								$trx_fee_disc = ($amount*$sales_order_data["discount_rate"])/100;
							...
							*/
							$trxfee= '<tr>'.
							'<td colspan="3"><span class="pull-right">'.$value["product_desc"].'</span></td>'.
							'<td>('.number_format($amount,2,".",",").')</td>'.
							'</tr>';
				...
				<tfoot>
				<tr><td colspan="3"><span class="pull-right">Discount(<?php echo (isset($sales_order_data["discount_rate"]))?$sales_order_data["discount_rate"]:"0";?>%)</span></td>
					<td>
					<?php 
					if($tax=="Inclusive")
					{
				...
				<?php if(count($credit_note_data)>0){?>
					...
					<?php 
					$creditnote_amount = $value["total_amount"];
					if($value["credit_type"]==2)
					{
						$creditnote_amount = ($value["total_amount"] - $value["discount"]);
					}
					$total_amount = $total_amount - $creditnote_amount;
					?>
					<tr><td colspan="3"><span class="pull-right">Less Credit <a target="_blank" href="index.php?r=pin/viewcn&id=<?php echo $value["credit_note_id"];?>">Credit Note</a></span></td><td><?php echo number_format($creditnote_amount,"2",".",",");?></td></tr>
					<?php } ?>
